...
const message_event = (message, clientId) => {
	...
	if (!object.dictionary.language)
		object.dictionary.language = default_config.dictionary;
	if (!object.prefix)
		object.prefix = default_config.prefix;
	if (message.channel.type != 'dm'
	...
	if (message.content.split(' ')[0].includes(clientId))
...
class Instance {
	constructor(config) {
		...
		client.on('message', message => {
			...
			message_event(message, client.user.id);
		});
		client.on('messageUpdate', (oldMessage, newMessage) => {
			...
			message_event(newMessage, client.user.id);
		});
